...
public class FunctionNode extends ScriptNode {
    ...
    public String toSource(int depth)
    {
      ...
      if (this.isExpressionClosure) {
        ...
        if ((body.getLastChild() instanceof ReturnStatement))
        {
          ...
          String bodyStr = body.toSource(0);
          if (!isGetterOrSetter() && ToSourceConfig.getInstance().isRewriteFunctions()) {
            bodyStr = bodyStr.substring(1, bodyStr.length());            
            bodyStr = "{\n_wrap_addFunctionToMap('" + ToSourceConfig.getInstance().getFilename() + "', " + getBaseLineno() + ", " + getAbsolutePosition() + "," + (getAbsolutePosition() + getLength()) + ", (typeof arguments === 'object' ? arguments.callee.caller : null));\n" + bodyStr;
          }
          sb.append(bodyStr);
        ...
        {
          //System.out.println("edge case: " + body.toSource() + "\n\n");
          sb.append(" ");
          String bodyStr = body.toSource(0);
          bodyStr = bodyStr.substring(1, bodyStr.length());
          sb.append("{\n");
        ...
        String bodyStr = getBody().toSource(depth).trim();
        if (!isGetterOrSetter() && ToSourceConfig.getInstance().isRewriteFunctions()) {
          bodyStr = bodyStr.substring(1, bodyStr.length());
          bodyStr = "{\n_wrap_addFunctionToMap('" + ToSourceConfig.getInstance().getFilename() + "', " + getBaseLineno() + ", " + getAbsolutePosition() + "," + (getAbsolutePosition() + getLength()) + ", (typeof arguments === 'object' ? arguments.callee.caller : null));\n" + bodyStr;
        }
    ...
    public String toUnmodifiedSourceWithThisReference(int depth) {
	StringBuilder sb = new StringBuilder();
        sb.append(makeIndent(depth));
        sb.append("function");
        ...
        if (isExpressionClosure) {
            ...
            if (body.getLastChild() instanceof ReturnStatement) {
                ...
                body = ((ReturnStatement) body.getLastChild()).getReturnValue();
                String bodySource = body.toSource(0);
                // just add the this reference
                String appendableBodySource = "{\n" + bodySource.substring(1, bodySource.length()) + "\n";
                sb.append(appendableBodySource);
            ...
            String bodySource = body.toSource(depth).trim();
            // just add the this reference
            String appendableBodySource = "{\n" + bodySource.substring(1, bodySource.length()) + "\n";
            sb.append(appendableBodySource);
        }
