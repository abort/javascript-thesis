...
Array.implement({copy: function(start, length) {
...
var Events = new Class({addEvent: function(type, fn, internal) {
  if (fn != $empty) {
...
Window.implement({$: function(el, nocash) {
...
Element.Inserters = new Hash({before: function(context, element) {
...
(function() {
  ...
  Element.implement({getPrevious: function(match, nocash) {
...
Element.implement({setOpacity: function(op) {
...
Selectors.Utils = {chk: function(item, uniques) {
  ...
  while ((m = Selectors.RegExps.combined.exec(selector))) {
    ...
    if (cn) {
      ...
      if (parser) 
        parsed.pseudos.push({parser: parser, argument: pa});
      else 
...
Native.implement([Element, Document], {getElementsByClassName: function(className) {
  return this.getElements('.' + className);
}, getElementsBySelector: function(selector) {
  return this.getElements(selector);
}});
Elements.implement({filterByTag: function(tag) {
...
var Fx = new Class({Implements: [Chain, Events, Options], options: {fps: 50, unit: false, duration: 500, link: 'ignore', transition: function(p) {
  ...
  if (!this.callChain()) 
    this.fireEvent('onChainComplete', this.subject);
}, onCancel: function() {
...
Fx.implement({custom: function(from, to) {
...
(function() {
  ...
  if (typeof trans == 'string' && (trans = trans.split(':'))) {
    ...
    if (trans[1]) 
      base = base['ease' + trans[1].capitalize() + (trans[2] ? trans[2].capitalize() : '')];
...
var Request = new Class({Implements: [Chain, Events, Options], options: {url: '', data: '', headers: {'X-Requested-With': 'XMLHttpRequest', 'Accept': 'text/javascript, text/html, application/xml, text/xml, */*'}, async: true, method: 'post', link: 'ignore', isSuccess: null, emulation: true, urlEncoded: true, encoding: 'utf-8', evalScripts: false, evalResponse: false}, initialize: function(options) {
  ...
  return true;
    ...
    this.fireEvent('onException', [key, value]);
}, this);
...
var XHR = new Class({Extends: Request, options: {update: false}, initialize: function(url, options) {
  this.parent(options);
  this.url = url;
}, request: function(data) {
  return this.send(this.url, data || this.options.data);
}, send: function(url, data) {
  if (!this.check(arguments.callee, url, data)) 
    return this;
  return this.parent({url: url, data: data});
}, success: function(text, xml) {
  text = this.processScripts(text);
  if (this.options.update) 
    $(this.options.update).empty().set('html', text);
...
JSON.Remote = new Class({options: {key: 'json'}, Extends: Request.JSON, initialize: function(url, options) {
  this.parent(options);
  this.onComplete = $empty;
  this.url = url;
}, send: function(data) {
  if (!this.check(arguments.callee, data)) 
    return this;
  return this.parent({url: this.url, data: {json: Json.encode(data)}});
}, failure: function() {
  this.fireEvent('onFailure', this.xhr);
}});
Fx.Slide = new Class({Extends: Fx, options: {mode: 'vertical'}, initialize: function(element, options) {
  ...
  this.wrapper = wrapper || new Element('div', {styles: $extend(this.element.getStyles('margin', 'position'), {'overflow': 'hidden'})}).wraps(this.element);
...
Fx.Scroll = new Class({Extends: Fx, options: {offset: {'x': 0, 'y': 0}, wheelStops: true}, initialize: function(element, options) {
  this.element = this.subject = $(element);
  this.parent(options);
...
Fx.Elements = new Class({Extends: Fx.CSS, initialize: function(elements, options) {
...
Drag.Move = new Class({Extends: Drag, options: {droppables: [], container: false}, initialize: function(element, options) {
  ...
  if (this.overed != overed) {
    if (this.overed) 
      this.fireEvent('onLeave', [this.element, this.overed]);
    if (overed) {
      this.overed = overed;
...
var Color = new Native({initialize: function(color, type) {
...
Hash.Cookie = new Class({Extends: Cookie, options: {autoSave: true}, initialize: function(name, options) {
...
var Sortables = new Class({Implements: [Events, Options], options: {snap: 4, opacity: 1, clone: false, revert: false, handle: false, constrain: false}, initialize: function(lists, options) {
  this.setOptions(options);
  this.elements = [];
  this.lists = [];
  this.idle = true;
  ...
  return element.clone(true).setStyles({'margin': '0px', 'position': 'absolute', 'visibility': 'hidden', 'width': element.getStyle('width')}).inject(this.list).position(element.getPosition(element.offsetParent));
...
var SmoothScroll = new Class({Extends: Fx.Scroll, initialize: function(options, context) {
...
var Slider = new Class({Implements: [Events, Options], options: {onTick: function(position) {
...
var Scroller = new Class({Implements: [Events, Options], options: {area: 20, velocity: 1, onChange: function(x, y) {
  ...
  if (change.y || change.x) 
    this.fireEvent('onChange', [scroll.x + change.x, scroll.y + change.y]);
}});
var Asset = new Hash({javascript: function(source, properties) {
  properties = $extend({onload: $empty, document: document, check: $lambda(true)}, properties);
  var script = new Element('script', {'src': source, 'type': 'text/javascript'});
  var load = properties.onload.bind(script), check = properties.check, doc = properties.document;
  delete properties.onload;
  delete properties.check;
  delete properties.document;
  script.addEvents({load: load, readystatechange: function() {
  ...
  properties = $merge({'onload': $empty, 'onabort': $empty, 'onerror': $empty}, properties);
  var image = new Image();
  ...
  options = $merge({onComplete: $empty, onProgress: $empty}, options);
  if (!sources.push) 
    sources = [sources];
  var images = [];
  var counter = 0;
