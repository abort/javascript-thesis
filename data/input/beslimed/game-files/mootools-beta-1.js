...
(function (objects) {
    for (var name in objects) new Native({
        name: name,
...
(function () {
    ...
    Native.implement([Document, Window], {
        getSize: function () {
            ...
            if (Browser.Engine.presto || Browser.Engine.webkit) return {
                x: win.innerWidth,
                y: win.innerHeight
            };
...
Fx.Style = function (element, property, options) {
    return new Fx.Tween(element, $extend({
        property: property
    }, options));
};
Element.implement({
...
var Drag = new Class({
    ...
    start: function (event) {
        ...
        for (var z in this.options.modifiers) {
            ...
            else this.value.now[z] = this.element[this.options.modifiers[z]]; if (this.options.invert) this.value.now[z] *= -1;
...
Drag.Move = new Class({
    ...
    start: function (event) {
        if (this.container) {
            var el = this.element,
                cont = this.container,
                ccoo = cont.getCoordinates(el.offsetParent),
                cps = {},
                ems = {};
            ...
            var width = el.offsetWidth + ems.left + ems.right,
                height = el.offsetHeight + ems.top + ems.bottom;
