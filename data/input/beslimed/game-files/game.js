var gameOptions = {Implements: [Options, Chain], options: {width: 8, height: 8, tilewidth: 32, tileheight: 32, fieldid: 'field', pointsid: 'p', timeid: 't', timebar: 'timebar', levelid: 'l', hintid: 'h', tileid: 'tiles', reqtileid: 'ntiles', startid: 'start', infoid: 'info', colors: false, haslines: false, lines: false, needtile: false, nextempty: false, tile1: false, tile2: false, tile1bak: false, idle: false, tilecount: 0, points: 0, delay: 200, fps: 30, hint: false, time: 0, hintcount: 0, starthints: 3, periodical: false, gameover: 'GAME OVER', nohintsleft: 'No hints left', nomoves: 'No Moves left!', level: 0, levels: {0: {tiles: 4, time: 120, reqtile: 60, text: 'Good Job'}, 1: {tiles: 4, time: 110, reqtile: 70, text: 'Well Done'}, 2: {tiles: 5, time: 100, reqtile: 80, text: 'Wohooo'}, 3: {tiles: 5, time: 90, reqtile: 90, text: 'Now Level 4'}, 4: {tiles: 6, time: 80, reqtile: 100, text: 'You are good'}, 5: {tiles: 6, time: 60, reqtile: 110, text: 'Level 6 Done'}, 6: {tiles: 7, time: 50, reqtile: 120, text: 'Mooooooo'}, 7: {tiles: 7, time: 40, reqtile: 130, text: 'Nice Done'}, 8: {tiles: 8, time: 30, reqtile: 140, text: 'Ready ... GO'}, 9: {tiles: 8, time: 20, reqtile: 150, text: 'You are insane!'}}}, initialize: function(options) {
  ...
  for (var y = 0; y <= this.options.width - 1; y++) {
    for (var x = 0; x <= this.options.height - 1; x++) {
      ...
      this.tiles.set(tileid, new Element('DIV', {'id': tileid, 'class': 'outer', 'styles': {'position': 'absolute', 'width': this.options.tilewidth + 'px', 'height': this.options.tileheight + 'px', 'top': (y * this.options.tileheight) + 'px', 'left': (x * this.options.tilewidth) + 'px', 'z-index': 0}}));
      inner = new Element('DIV', {'class': 'inner', 'styles': {'width': '100%', 'height': '100%'}});
      inner.inject(this.tiles[tileid]);
      this.tiles[tileid].inject(this.options.fieldid);
      this.fx[tileid] = new Fx.Morph($('t' + tilecount), {duration: this.options.delay, wait: false, transition: Fx.Transitions.Circ.easeOut, fps: this.options.fps, onComplete: this.onFxComplete.bind(this), onStart: this.onFxStart.bind(this)});
      this.tiles[tileid].drag = new Drag.Move($(this.tiles[tileid]), {snap: 0, onBeforeStart: function(el) {
...
}.bind(this), onComplete: function(el) {
  var xx = el.getStyle("left").toInt();
  var yy = el.getStyle("top").toInt();
  ...
  for (var x = this.options.width - 1; x >= 0; x--) {
    for (var y = this.options.height - 1; y >= 0; y--) {
      ...
      $(tileid).setStyles({'top': (-this.options.tileheight) + 'px', 'left': (x * this.options.tilewidth) + 'px'});
  ...
  if (!tileid) 
    return false;
  this.fxplodetile[tileid] = new Element('DIV', {'id': "ex" + tileid, 'class': $(tileid).getProperty("class"), 'styles': {'position': 'absolute', 'width': this.options.tilewidth + 'px', 'height': this.options.tileheight + 'px', 'top': (y * this.options.tileheight) + 'px', 'left': (x * this.options.tilewidth) + 'px'}});
  this.fxplodetile[tileid].injectInside($(this.options.fieldid));
  ...
  this.fxplode[tileid] = new Fx.Morph($('ex' + tileid), {duration: this.options.delay * 4, wait: false, transition: Fx.Transitions.Circ.easeIn, fps: this.options.fps, onComplete: this.onFxplodeComplete.bind(this)});
  this.fxplode[tileid].start.delay(10, this.fxplode[tileid], {'top': yy, 'left': xx, 'opacity': 0.4});
  ...
  if (!id) 
    return false;
  if (y == 0) {
    $(id).setStyles({'top': (-this.options.tileheight) + 'px', 'left': (x * this.options.tilewidth) + 'px'});
    this.fx[id].start({'top': 0});
  } else {
    $(id).setStyles({'top': (y * this.options.tileheight) + 'px', 'left': (x * this.options.tilewidth) + 'px'});
  }
  ...
  for (var y = this.options.height - 2; y >= 0; y--) {
    for (var x = this.options.width - 1; x >= 0; x--) {
      if (!this.getTile(x, y + 1) && this.getTile(x, y)) {
        ...
        if (id != "false") {
          this.fx[id].start({'top': $(id).getStyle('top').toInt() + (this.options.tileheight * depth)});
  ...
  for (var x = 0; x <= this.options.height - 1; x++) {
    for (var y = 0; y <= this.options.width - 1; y++) {
      if (this.hasMoves({'x': x - 1, 'y': y}, {'x': x, 'y': y})) 
        return true;
      if (this.hasMoves({'x': x + 1, 'y': y}, {'x': x, 'y': y})) 
        return true;
      if (this.hasMoves({'x': x, 'y': y - 1}, {'x': x, 'y': y})) 
        return true;
      if (this.hasMoves({'x': x, 'y': y + 1}, {'x': x, 'y': y})) 
        return true;
    }
  ...
  this.fxp[x + "_" + y] = new Element('DIV', {'id': "p" + x + "_" + y, 'class': 'points', 'styles': {'position': 'absolute', 'width': 32, 'height': 20, 'top': (y * this.options.tileheight) + 8 + 'px', 'left': (x * this.options.tilewidth) + 'px'}});
  this.fxp[x + "_" + y].injectInside($(this.options.fieldid));
  this.fxp[x + "_" + y].innerHTML = points;
  this.fxp[x + "_" + y] = new Fx.Morph($("p" + x + "_" + y), {duration: this.options.delay * 10, wait: false, transition: Fx.Transitions.Sine.easeInOut, fps: this.options.fps, onComplete: this.onFxplodeComplete.bind(this)});
  this.fxp[x + "_" + y].start({'top': ((y * this.options.tileheight) + 8 - (this.options.tileheight * 2)) + 'px', 'opacity': 0.4});
}};
var jewel = new Class(gameOptions);
